-- ========================================
-- SCRIPT DE SINCRONIZACIÓN (Generado: 3/27/2026 5:58:27 PM)
-- ========================================


-- Agregar columna: fza_articulos.TIPO_VARIACION_ARTICULO
ALTER TABLE `fza_articulos` ADD COLUMN `TIPO_VARIACION_ARTICULO` varchar(20) NULL DEFAULT NULL;

-- Agregar índice: fza_articulos.TIPO_VARIACION_ARTICULO
ALTER TABLE `fza_articulos` ADD INDEX `TIPO_VARIACION_ARTICULO` (`TIPO_VARIACION_ARTICULO`);

-- Modificar columna: fza_articulos_conjuntos_asign.CODIGO_ARTICULO_ACA
UPDATE `fza_articulos_conjuntos_asign` SET `CODIGO_ARTICULO_ACA` = LEFT(`CODIGO_ARTICULO_ACA`, 20) WHERE LENGTH(`CODIGO_ARTICULO_ACA`) > 20;
UPDATE `fza_articulos_conjuntos_asign` SET `CODIGO_ARTICULO_ACA` = '' WHERE `CODIGO_ARTICULO_ACA` IS NULL;
ALTER TABLE `fza_articulos_conjuntos_asign` MODIFY COLUMN `CODIGO_ARTICULO_ACA` varchar(20) NOT NULL COMMENT 'FK fza_articulos';

-- Modificar columna: fza_articulos_conjuntos_asign.ID_CONJUNTO_ACA
UPDATE `fza_articulos_conjuntos_asign` SET `ID_CONJUNTO_ACA` = '' WHERE `ID_CONJUNTO_ACA` IS NULL;
ALTER TABLE `fza_articulos_conjuntos_asign` MODIFY COLUMN `ID_CONJUNTO_ACA` int(11) NOT NULL COMMENT 'FK fza_atributos_conjuntos';

-- Modificar columna: fza_articulos_conjuntos_asign.ID_ATRIBUTO_ACA
UPDATE `fza_articulos_conjuntos_asign` SET `ID_ATRIBUTO_ACA` = LEFT(`ID_ATRIBUTO_ACA`, 20) WHERE LENGTH(`ID_ATRIBUTO_ACA`) > 20;
UPDATE `fza_articulos_conjuntos_asign` SET `ID_ATRIBUTO_ACA` = '' WHERE `ID_ATRIBUTO_ACA` IS NULL;
ALTER TABLE `fza_articulos_conjuntos_asign` MODIFY COLUMN `ID_ATRIBUTO_ACA` varchar(20) NOT NULL COMMENT 'Posición del atributo (CO, TAL...) – denormalizado del conjunto para índice directo';

-- Modificar columna: fza_articulos_conjuntos_asign.ES_GENERACION_AUTO
UPDATE `fza_articulos_conjuntos_asign` SET `ES_GENERACION_AUTO` = LEFT(`ES_GENERACION_AUTO`, 1) WHERE LENGTH(`ES_GENERACION_AUTO`) > 1;
UPDATE `fza_articulos_conjuntos_asign` SET `ES_GENERACION_AUTO` = '' WHERE `ES_GENERACION_AUTO` IS NULL;
ALTER TABLE `fza_articulos_conjuntos_asign` MODIFY COLUMN `ES_GENERACION_AUTO` char(1) NOT NULL DEFAULT 'S' COMMENT 'S=genera SKUs automáticamente al procesar';

-- Modificar columna: fza_articulos_conjuntos_asign.INSTANTEMODIF
UPDATE `fza_articulos_conjuntos_asign` SET `INSTANTEMODIF` = '' WHERE `INSTANTEMODIF` IS NULL;
ALTER TABLE `fza_articulos_conjuntos_asign` MODIFY COLUMN `INSTANTEMODIF` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE CURRENT_TIMESTAMP;

-- Modificar columna: fza_articulos_conjuntos_asign.INSTANTEALTA
UPDATE `fza_articulos_conjuntos_asign` SET `INSTANTEALTA` = '' WHERE `INSTANTEALTA` IS NULL;
ALTER TABLE `fza_articulos_conjuntos_asign` MODIFY COLUMN `INSTANTEALTA` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00';

-- Modificar columna: fza_articulos_conjuntos_asign.USUARIOALTA
UPDATE `fza_articulos_conjuntos_asign` SET `USUARIOALTA` = LEFT(`USUARIOALTA`, 100) WHERE LENGTH(`USUARIOALTA`) > 100;
UPDATE `fza_articulos_conjuntos_asign` SET `USUARIOALTA` = '' WHERE `USUARIOALTA` IS NULL;
ALTER TABLE `fza_articulos_conjuntos_asign` MODIFY COLUMN `USUARIOALTA` varchar(100) NOT NULL;

-- Modificar columna: fza_articulos_conjuntos_asign.USUARIOMODIF
UPDATE `fza_articulos_conjuntos_asign` SET `USUARIOMODIF` = LEFT(`USUARIOMODIF`, 100) WHERE LENGTH(`USUARIOMODIF`) > 100;
UPDATE `fza_articulos_conjuntos_asign` SET `USUARIOMODIF` = '' WHERE `USUARIOMODIF` IS NULL;
ALTER TABLE `fza_articulos_conjuntos_asign` MODIFY COLUMN `USUARIOMODIF` varchar(100) NOT NULL;

-- Agregar índice: fza_articulos_conjuntos_asign.IDX_ACA_ATRIBUTO
ALTER TABLE `fza_articulos_conjuntos_asign` ADD INDEX `IDX_ACA_ATRIBUTO` (`CODIGO_ARTICULO_ACA`, `ID_ATRIBUTO_ACA`);

-- Modificar índice: fza_articulos_conjuntos_asign.PRIMARY
SET @pk_exists := (SELECT COUNT(*) FROM information_schema.table_constraints WHERE table_schema = DATABASE() AND table_name = 'fza_articulos_conjuntos_asign' AND constraint_type = 'PRIMARY KEY');
SET @sql_drop := IF(@pk_exists > 0, 'ALTER TABLE `fza_articulos_conjuntos_asign` DROP PRIMARY KEY', 'SELECT "No Primary Key to drop"');
PREPARE stmt FROM @sql_drop;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Descomenta para limpieza de duplicados previa a la creación de PK
-- DELETE FROM `fza_articulos_conjuntos_asign` WHERE -- (`CODIGO_ARTICULO_ACA`, `ID_CONJUNTO_ACA`) IN (-- SELECT `CODIGO_ARTICULO_ACA`, `ID_CONJUNTO_ACA` FROM (-- SELECT `CODIGO_ARTICULO_ACA`, `ID_CONJUNTO_ACA`-- FROM `fza_articulos_conjuntos_asign`-- GROUP BY `CODIGO_ARTICULO_ACA`, `ID_CONJUNTO_ACA`-- HAVING COUNT(*) > 1-- ) AS c-- );

ALTER TABLE `fza_articulos_conjuntos_asign` ADD PRIMARY KEY (`CODIGO_ARTICULO_ACA`, `ID_CONJUNTO_ACA`);

-- Agregar columna: fza_articulos_propiedades.CODIGO_ARTICULO
ALTER TABLE `fza_articulos_propiedades` ADD COLUMN `CODIGO_ARTICULO` varchar(20) NOT NULL;

-- Agregar columna: fza_articulos_propiedades.CODIGO_PROPIEDAD
ALTER TABLE `fza_articulos_propiedades` ADD COLUMN `CODIGO_PROPIEDAD` varchar(20) NOT NULL COMMENT 'FK a fza_propiedades (ej: MATERIAL, EDAD_MAX)';

-- Agregar columna: fza_articulos_propiedades.ID_VALOR_PV
ALTER TABLE `fza_articulos_propiedades` ADD COLUMN `ID_VALOR_PV` int(11) NULL DEFAULT NULL COMMENT 'Si es tipo LISTA, guardas aquí el ID de fza_propiedades_valores';

-- Agregar columna: fza_articulos_propiedades.VALOR_LIBRE
ALTER TABLE `fza_articulos_propiedades` ADD COLUMN `VALOR_LIBRE` varchar(255) NULL DEFAULT NULL COMMENT 'Si es tipo TEXTO o NUMERO, escribes el dato directamente aquí';

-- Eliminar columna: fza_articulos_propiedades.CODIGO_ARTICULO_AP
ALTER TABLE `fza_articulos_propiedades` DROP COLUMN `CODIGO_ARTICULO_AP`;

-- Eliminar columna: fza_articulos_propiedades.ID_VALOR_AP
ALTER TABLE `fza_articulos_propiedades` DROP COLUMN `ID_VALOR_AP`;

-- Eliminar índice: fza_articulos_propiedades.IDX_VALOR_AP
ALTER TABLE `fza_articulos_propiedades` DROP INDEX `IDX_VALOR_AP`;

-- Agregar índice: fza_articulos_propiedades.IDX_VALOR_LISTA
ALTER TABLE `fza_articulos_propiedades` ADD INDEX `IDX_VALOR_LISTA` (`ID_VALOR_PV`);

-- Modificar índice: fza_articulos_propiedades.PRIMARY
SET @pk_exists := (SELECT COUNT(*) FROM information_schema.table_constraints WHERE table_schema = DATABASE() AND table_name = 'fza_articulos_propiedades' AND constraint_type = 'PRIMARY KEY');
SET @sql_drop := IF(@pk_exists > 0, 'ALTER TABLE `fza_articulos_propiedades` DROP PRIMARY KEY', 'SELECT "No Primary Key to drop"');
PREPARE stmt FROM @sql_drop;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Descomenta para limpieza de duplicados previa a la creación de PK
-- DELETE FROM `fza_articulos_propiedades` WHERE -- (`CODIGO_ARTICULO`, `CODIGO_PROPIEDAD`) IN (-- SELECT `CODIGO_ARTICULO`, `CODIGO_PROPIEDAD` FROM (-- SELECT `CODIGO_ARTICULO`, `CODIGO_PROPIEDAD`-- FROM `fza_articulos_propiedades`-- GROUP BY `CODIGO_ARTICULO`, `CODIGO_PROPIEDAD`-- HAVING COUNT(*) > 1-- ) AS c-- );

ALTER TABLE `fza_articulos_propiedades` ADD PRIMARY KEY (`CODIGO_ARTICULO`, `CODIGO_PROPIEDAD`);

-- Agregar columna: fza_articulos_proveedores.REF_PROVEEDOR_ARTICULO_PROVEEDOR
ALTER TABLE `fza_articulos_proveedores` ADD COLUMN `REF_PROVEEDOR_ARTICULO_PROVEEDOR` varchar(100) NULL DEFAULT NULL;

-- Agregar columna: fza_articulos_proveedores.REFPROVEEDOR_AP
ALTER TABLE `fza_articulos_proveedores` ADD COLUMN `REFPROVEEDOR_AP` varchar(50) NULL DEFAULT NULL;

-- Tabla nueva: fza_atributos_basicos
...
-- Agregar índice: fza_atributos_basicos.ID_VA_BASICO
ALTER TABLE `fza_atributos_basicos` ADD UNIQUE INDEX `ID_VA_BASICO` (`ID_VA_BASICO`, `CODIGO_BASICO`);

-- Agregar columna: fza_atributos_conjuntos.ID_VARIACION_AC
ALTER TABLE `fza_atributos_conjuntos` ADD COLUMN `ID_VARIACION_AC` varchar(20) NOT NULL COMMENT 'FK fza_variaciones (TC, TEMP…)';

-- Agregar columna: fza_atributos_conjuntos.ID_ATRIBUTO_AC
ALTER TABLE `fza_atributos_conjuntos` ADD COLUMN `ID_ATRIBUTO_AC` varchar(20) NULL DEFAULT NULL COMMENT 'Posición/atributo dentro de la variación (CO, TAL, TEMP…) → FK fza_variaciones_atributos';

-- Eliminar columna: fza_atributos_conjuntos.ID_VA_AC
ALTER TABLE `fza_atributos_conjuntos` DROP COLUMN `ID_VA_AC`;

-- Modificar índice: fza_atributos_conjuntos.IDX_VAR_AC
...
-- Modificar columna: fza_caja_operaciones.TIPO_OPERACION_OPCAJA
UPDATE `fza_caja_operaciones` SET `TIPO_OPERACION_OPCAJA` = LEFT(`TIPO_OPERACION_OPCAJA`, 2) WHERE LENGTH(`TIPO_OPERACION_OPCAJA`) > 2;
UPDATE `fza_caja_operaciones` SET `TIPO_OPERACION_OPCAJA` = '' WHERE `TIPO_OPERACION_OPCAJA` IS NULL;
ALTER TABLE `fza_caja_operaciones` MODIFY COLUMN `TIPO_OPERACION_OPCAJA` varchar(2) NOT NULL COMMENT 'VE, AL, CB, EC, GC, TR, AT';

-- Agregar columna: fza_clientes.TOTAL_LIMITE_CREDITO_CLIENTE
ALTER TABLE `fza_clientes` ADD COLUMN `TOTAL_LIMITE_CREDITO_CLIENTE` decimal(19,6) NULL DEFAULT NULL;

-- Agregar columna: fza_clientes.ESPERMITE_DEUDA_CLIENTE
ALTER TABLE `fza_clientes` ADD COLUMN `ESPERMITE_DEUDA_CLIENTE` varchar(1) NULL DEFAULT NULL;

-- Agregar columna: fza_clientes.TOTAL_DEUDA_CLIENTE
ALTER TABLE `fza_clientes` ADD COLUMN `TOTAL_DEUDA_CLIENTE` decimal(19,6) NULL DEFAULT NULL;

-- Modificar índice: fza_contadores.PRIMARY
SET @pk_exists := (SELECT COUNT(*) FROM information_schema.table_constraints WHERE table_schema = DATABASE() AND table_name = 'fza_contadores' AND constraint_type = 'PRIMARY KEY');
SET @sql_drop := IF(@pk_exists > 0, 'ALTER TABLE `fza_contadores` DROP PRIMARY KEY', 'SELECT "No Primary Key to drop"');
PREPARE stmt FROM @sql_drop;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Descomenta para limpieza de duplicados previa a la creación de PK
-- DELETE FROM `fza_contadores` WHERE -- (`TIPODOC_CONTADOR`, `EMPRESA_CONTADOR`, `SERIE_CONTADOR`) IN (-- SELECT `TIPODOC_CONTADOR`, `EMPRESA_CONTADOR`, `SERIE_CONTADOR` FROM (-- SELECT `TIPODOC_CONTADOR`, `EMPRESA_CONTADOR`, `SERIE_CONTADOR`-- FROM `fza_contadores`-- GROUP BY `TIPODOC_CONTADOR`, `EMPRESA_CONTADOR`, `SERIE_CONTADOR`-- HAVING COUNT(*) > 1-- ) AS c-- );

ALTER TABLE `fza_contadores` ADD PRIMARY KEY (`TIPODOC_CONTADOR`, `EMPRESA_CONTADOR`, `SERIE_CONTADOR`);

-- Tabla nueva: fza_depositos_cliente
...
-- Agregar índice: fza_depositos_cliente.IDX_DEP_CLIENTE
ALTER TABLE `fza_depositos_cliente` ADD INDEX `IDX_DEP_CLIENTE` (`CODIGO_CLIENTE_DEP`, `ESTADO_DEP`);

-- Agregar índice: fza_depositos_cliente.IDX_DEP_UNIDAD
ALTER TABLE `fza_depositos_cliente` ADD INDEX `IDX_DEP_UNIDAD` (`CODIGO_UNIDAD_DEP`, `ESTADO_DEP`);

-- Agregar columna: fza_empresas_series.SUBTIPO_SERIE
ALTER TABLE `fza_empresas_series` ADD COLUMN `SUBTIPO_SERIE` varchar(20) NULL DEFAULT NULL;

-- Eliminar columna: fza_empresas_series.SUBITPO_SERIE
ALTER TABLE `fza_empresas_series` DROP COLUMN `SUBITPO_SERIE`;

-- Tabla nueva: fza_familias_atributos
...
-- Agregar índice: fza_familias_atributos.IDX_FAM_ATR_ATR
ALTER TABLE `fza_familias_atributos` ADD INDEX `IDX_FAM_ATR_ATR` (`CODIGO_PROPIEDAD`);

-- Tabla nueva: fza_familias_atributos_defecto
...
-- Tabla nueva: fza_familias_claves_info_defecto
...
-- Tabla nueva: fza_propiedades_valores
...
-- Agregar índice: fza_propiedades_valores.IDX_PROPIEDAD_PV
ALTER TABLE `fza_propiedades_valores` ADD INDEX `IDX_PROPIEDAD_PV` (`ID_PROPIEDAD_PV`);

-- Tabla nueva: fza_proveedores_familias
...
-- Tabla nueva: fza_proveedores_familias_conjuntos
...
-- Eliminar columna: fza_variaciones_atributos.ESDEFINITORIO
ALTER TABLE `fza_variaciones_atributos` DROP COLUMN `ESDEFINITORIO`;
...
CREATE ALGORITHM=UNDEFINED  VIEW `vi_articulos_skus_extendida` AS select `sku`.`CODIGO_UNIDAD_SKU` AS `CODIGO_UNIDAD_SKU`,`sku`.`CODIGO_ARTICULO_SKU` AS `CODIGO_ARTICULO_SKU`,`sku`.`CODIGO_VAR_SKU` AS `CODIGO_VAR_SKU`,`sku`.`ESACTIVO_SKU` AS `ESACTIVO_SKU`,`cb`.`CODIGO_BARRAS_CB` AS `CODIGO_BARRAS_CB`,coalesce(`stk`.`STOCK_TOTAL`,0) AS `STOCK_TOTAL`,`sku`.`INSTANTEMODIF` AS `INSTANTEMODIF`,`sku`.`INSTANTEALTA` AS `INSTANTEALTA`,`sku`.`USUARIOALTA` AS `USUARIOALTA`,`sku`.`USUARIOMODIF` AS `USUARIOMODIF` from ((`factuzam`.`fza_articulos_skus` `sku` left join (select `factuzam`.`fza_codigos_barras`.`CODIGO_UNIDAD_CB` AS `CODIGO_UNIDAD_CB`,max(`factuzam`.`fza_codigos_barras`.`CODIGO_BARRAS_CB`) AS `CODIGO_BARRAS_CB` from `factuzam`.`fza_codigos_barras` group by `factuzam`.`fza_codigos_barras`.`CODIGO_UNIDAD_CB`) `cb` on(`sku`.`CODIGO_UNIDAD_SKU` = `cb`.`CODIGO_UNIDAD_CB`)) left join (select `factuzam`.`fza_articulos_stockactual`.`CODIGO_UNIDAD_STK` AS `CODIGO_UNIDAD_STK`,sum(`factuzam`.`fza_articulos_stockactual`.`CANTIDAD_STK`) AS `STOCK_TOTAL` from `factuzam`.`fza_articulos_stockactual` group by `factuzam`.`fza_articulos_stockactual`.`CODIGO_UNIDAD_STK`) `stk` on(`sku`.`CODIGO_UNIDAD_SKU` = `stk`.`CODIGO_UNIDAD_STK`));
